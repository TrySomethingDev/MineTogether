...
commandBase.registerCommand("startfishing", async ({ client, user }) => {
  // Check user is not already fishing
  const isFishing = await db.query.fishingSpots.findFirst({
  ...
  if (isFishing) {
    return client.say(
      channelName,
      `Sorry ${user.name}, you are already fishing.`,
    );
  }
